---
layout: default
title: Alfarizqi Wira Anadyar
description: Information Systems Student | Data Analyst & Local LLM Enthusiast
---

# 👋 Hello, I'm Alfarizqi Wira Anadyar

<img src="IMG_1163.png" alt="Profile Photo" width="200" style="border-radius: 50%;">

**Information Systems Student @ Universitas Trisakti (GPA 3.21)**
📍 Jakarta, Indonesia | 📧 farizanadyar@gmail.com | 🔗 [LinkedIn Profile](https://www.linkedin.com/in/alfarizqi-wira-anadyar-b52ab9248/)

I am a final-year student passionate about bridging the gap between **Business Process** and **Technical Implementation**. Currently focusing on **Data Analytics** and **On-Premise AI Solutions**.

---

## 💻 Technical Stack & "Home Lab"

I don't just learn theory; I build and run systems on my own high-performance workstation.

* **Languages:** Python (Pandas/Scikit-Learn), SQL (PostgreSQL/MySQL), Java (OOP), PHP.
* **Data & AI:** Power BI, Pentaho (ETL), Ollama (Local LLM), RAG Implementation.
* **Infrastructure:**
    * **My Local Rig:** AMD Ryzen 7500F + **Radeon RX 6800 (16GB VRAM)**.
    * *Why this matters:* I develop and fine-tune Large Language Models (Llama 3, Dolphin) locally, ensuring data privacy and deep understanding of hardware resource management.

---

## 🚀 Key Projects

### 1. Trisakti Academic QA System (RAG + Local LLM)
*Developing a privacy-focused chatbot for university academic services.*
* **Tech:** Python, LangChain, Ollama (Llama 3), FAISS Vector DB.
* **Highlights:**
    * Built a Retrieval-Augmented Generation (RAG) system to answer student queries using official campus documents.
    * Optimized inference speed on AMD GPU environment.
    * **Achievement:** Attended "AI for Social Transformation" Seminar to validate concepts [View Certificate](https://github.com/vangguard10/alfarizqi-portofolio/blob/main/Sertif%20LLM%20Peserta-6.pdf).

### 2. Heart Disease Prediction & Analysis
*End-to-end data analysis project to identify health risk factors.*
* **Tech:** Python (Pandas), Scikit-Learn, Power BI.
* **Highlights:**
    * Analyzed 900+ patient records to find correlations between age, cholesterol, and heart risk.
    * Created an interactive dashboard for medical insight visualization.
    * [Link to Dashboard/Screenshot](https://github.com/vangguard10/alfarizqi-portofolio/blob/main/image.png)

### 3. Northwind Data Warehouse (ETL Pipeline)
*Designing a robust data warehouse for sales analytics.*
* **Tech:** Pentaho Data Integration (Spoon), MySQL, Power BI.
* **Highlights:**
    * Designed a **Star Schema** to optimize query performance for reporting.
    * Built ETL jobs to transform raw transactional data into dimension and fact tables.
    * [View ETL Job Flow](https://github.com/vangguard10/alfarizqi-portofolio/blob/main/image7.png) | [View Star Schema Model](https://github.com/vangguard10/alfarizqi-portofolio/blob/main/image2.png)

---

## 🤝 Organizational Experience

### 🏛️ Badan Eksekutif Mahasiswa (BEM) FTI Trisakti
**Head of Communication & Information Department (Dec 2024 - Dec 2025)**
* Leading a creative team of **11 staff** to manage the official **@bemftiusakti** account, serving as the primary public face for the organization.
* Responsible for strategic communication and employer branding for an organization consisting of **85 active members**.
* Directed visual documentation for major faculty events and ensured consistent information flow to students.
* *Previous Role:* Staff of Kominfo (Oct 2023 - Sep 2024).
* **Achievement:** Awarded as **"Staff Terbaik" (Best Staff)** for outstanding dedication and competence in the Education field during the 2023/2024 tenure.
* 📄 [View Best Staff Award](https://github.com/vangguard10/alfarizqi-portofolio/blob/main/WhatsApp%20Image%202026-01-11%20at%2018.17.37.jpeg)
* 📄 [View Staff Certificate](https://github.com/vangguard10/alfarizqi-portofolio/blob/main/Alfarizqi%20Wira%20Anadyar%20-%20Kominfo.pdf)

### 💻 HMTIF Universitas Trisakti
**Staff of Talent & Interest Department (Sep 2024 - Present)**
* Facilitating student engagement through e-sports competitions and talent development events.
* Actively supporting student welfare initiatives within the Informatics Engineering association.

### 🚀 IT Club SMAN 31 Jakarta
**Club President (2020 - 2021)**
* Led a community of tech enthusiasts, organizing coding workshops (HTML/Java) and digital design competitions.
* Mentored junior members in basic programming logic.

---

## 🏆 Certifications & Academic Achievements

### 👨‍🏫 Laboratory Assistant (Asisten Dosen)
**Jurusan Teknik Informatika, Universitas Trisakti (2024/2025)**
Selected as a mentor due to high academic performance in programming courses.
* **Algorithms & Programming:** Guided students in fundamental logic and coding structures. 
    * 📄 [View Algorithm Certificate](https://github.com/vangguard10/alfarizqi-portofolio/blob/main/Alfarizqi%20Wira%20Anadyar%20(1).png)
* **Object Oriented Programming (OOP):** Mentored students in Java concepts and graded practical exams. 
    * 📄 [View OOP Certificate](https://github.com/vangguard10/alfarizqi-portofolio/blob/main/Pemrograman%20-Alfarizqi%20Wira%20Anadyar.png)

### 🎨 Creative Team Committee - Bebras Indonesia Challenge 2024
**Biro Universitas Trisakti**
* Contributed to the creative direction of a national computational thinking competition.
* Managed visual assets and event documentation to support the event's success.
* 📄 [View Committee Certificate](./Bebras%20Committee%20Certificate%202024%20-%20Alfarizqi%20Wira%20Anadyar.pdf)

### 💻 Certified Web Developer
**Computer Learning Center INTERNUSA (Jul - Nov 2023)**
* Completed an intensive "Web Design & Web Programming" course with **Grade A (Excellent)** across all modules.
* Mastered full-stack fundamentals: **PHP, MySQL, Bootstrap, JavaScript/jQuery, and CSS**.
* Successfully deployed a dynamic web project with database integration as a final assessment.
* 📄 [View Certificate](https://github.com/vangguard10/alfarizqi-portofolio/blob/main/internusa.pdf)

---

### 📫 Let's Connect!
I am currently open for **Internship opportunities** in Data Analytics, IT Quality Assurance, or Tech Operations.

[Download My Resume (PDF)] | [LinkedIn Profile](https://www.linkedin.com/in/alfarizqi-wira-anadyar-b52ab9248/)
